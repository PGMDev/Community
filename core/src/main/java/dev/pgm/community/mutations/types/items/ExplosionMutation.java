...
import static dev.pgm.community.platform.CommunityEffects.EFFECTS;
...
public class ExplosionMutation extends KitMutationBase {
  ...
  private static final Material TNT_LAUNCHER_MATERIAL = CommunityMaterials.NETHER_BRICK_ITEM;
  ...
  public void onExplosiveLaunch(PlayerInteractEvent event) {
    ...
    if (item.getType() != CommunityMaterials.FIREBALL && item.getType() != TNT_LAUNCHER_MATERIAL)
      return;
  ...
  private static List<ItemStack> getTNTItems() {
    List<ItemStack> items = Lists.newArrayList(
        getTNTItem(), new ItemStack(CommunityMaterials.REDSTONE_TORCH_ON, TNT_SIZE.getValue()));
  ...
  private static ItemStack getFireballItem() {
    ItemStack item = new ItemBuilder()
        .material(CommunityMaterials.FIREBALL)
  ...
  private boolean launchExplosive(Player player, Material type) {
    ...
    if (type == CommunityMaterials.FIREBALL) {
