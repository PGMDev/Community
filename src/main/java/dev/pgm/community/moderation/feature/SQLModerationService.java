...
public class SQLModerationService extends SQLFeatureBase<Punishment> {
  ...
  private static final String CONSOLE_DB_NAME = "console"; // Used in issuer field when punishment issued by console
  ...
  public CompletableFuture<List<Punishment>> query(String target) {
    ...
    if (query.hasFetched()) {
      Community.log("Punishments were &acached&7, now returning cached results"); // TODO: Debug, remove
      return CompletableFuture.completedFuture(query.getPunishments());
    } else {
      Community.log("Punishments &cNOT FOUND&7 cached, now submitting queue to database..."); // TODO: Debug, remove
      return getDatabase()
