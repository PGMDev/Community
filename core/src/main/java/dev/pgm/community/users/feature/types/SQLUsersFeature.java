...
public class SQLUsersFeature extends UsersFeatureBase {
  ...
  public CompletableFuture<UserProfile> getStoredProfile(UUID id) {
    ...
    if (cached == null) {
      return service.query(id.toString()).thenApplyAsync(profile -> {
        if (profile != null) {
  ...
  public CompletableFuture<Optional<UUID>> getStoredId(String username) {
    ...
    if (!cached.isPresent()) {
      return service.query(username).thenApplyAsync(profile -> {
        UUID id = null;
  ...
  public void onLogin(PlayerJoinEvent event) {
    ...
    service.login(id, name, address).thenAcceptAsync(profile -> {
